;(function(window,document) {
	...
	window[fnName] = function(setConfig){
			...
			var obj = e.target;
				...
				if(obj.getAttribute('action')){
					...
					switch(obj.getAttribute('action'))
					{
						...
						//播放按钮
						case 'play':
								if(audioBtn.play.playStatus){
										$(".music-player__image").addClass("play");
										$(".music-player__pointer").addClass("play");
										$("#playerimg").addClass("img_rotate");
									}
									else{
										$(".music-player__image").removeClass("play");
										$(".music-player__pointer").removeClass("play");
										$("#playerimg").removeClass("img_rotate");
									}
		...
		this.loadFile = function(playStatus){
			...
			var newsrcs= "HATFANet/HATFANet_ListeningDemos/Demo"+datasrrc+"/"+song["types"]+"-"+$(".dmodel_hover")[0].dataset.title+".wav"
			...
			if(control.src){
				...
				operation = setTimeout(function(){
					...
					if(action){
						...
						$("#imgplay").attr("class",audioBtn.play.className);
					}else{
...
if(_default.song.length>songSelect){
